...
def test_metric_geometry_and_sl():
    """
    Test 1D metric geometry and Sturm–Liouville reduction for g = x².

    Verifies that the automatically derived symbolic quantities match their
    hand-computed values:

    * Inverse metric g¹¹ = x⁻².
    * Square-root determinant √g = x.
    * Christoffel symbol Γ¹₁₁ = 1/x.
    * Sturm–Liouville coefficient p = 1/x and weight w = x.
    """
    x = symbols('x', real=True, positive=True)
    m = Metric(x**2, (x,))
...
def test_integration_methods_consistency():
    """
    Test that symbolic and numerical volume integration agree for g = 1/x².

    The arc length ∫₁ᵉ (1/x) dx = log(e) − log(1) = 1.  Both the SymPy
    ``integrate`` path and the ``scipy.integrate.quad`` path must recover this
    value to within their respective numerical tolerances.
    """
    x = symbols('x', real=True, positive=True)
    m = Metric(1/x**2, (x,))
...
def test_geodesic_integrators_accuracy():
    """
    Test accuracy of all 1D geodesic integrators on the flat metric g = 1.

    For the flat metric Γ¹₁₁ = 0, so the geodesic equation reduces to ẍ = 0
    and the exact solution is x(t) = x₀ + v₀ t.  Starting from x₀ = 0,
    v₀ = 2.0 at t_end = 2.0, the expected endpoint is x = 4.0.

    Tolerances: 1e-5 for RK methods; 1e-1 for the symplectic (leapfrog) method,
    which introduces a small phase error at finite step size.
    """
    x = symbols('x', real=True)
    m = Metric(1, (x,))
...
def test_hamiltonian_flow_conservation():
    """
    Test Hamiltonian energy conservation for 1D symplectic integrators.

    Uses the cone metric g = x², whose Hamiltonian is H = p²/(2x²).  The
    initial momentum p₀ is chosen to give a non-trivial energy E₀ = p₀²/(2x₀²).
    All three symplectic-type methods ('verlet', 'stormer', 'symplectic_euler')
    must keep the relative standard deviation of the energy array below a
    method-specific tolerance over 2000 steps.

    Also asserts that the trajectory actually moves (|x(T) − x₀| > 0.1) to
    confirm the integrator is not trivially returning the initial state.
    """
...
def test_laplace_beltrami_properties():
    """
    Test the Laplace–Beltrami symbol for the 1D cone metric g = x².

    Verifies both the principal and subprincipal symbols against their
    hand-computed values:

    * Principal symbol: g¹¹ ξ² = ξ²/x².
    * Subprincipal symbol: (d/dx log√g) · g¹¹ · ξ = (1/x)(1/x²) ξ = ξ/x³.
    """
    x = symbols('x', real=True, positive=True)
    xi = symbols('xi', real=True)
    m = Metric(x**2, (x,))
...
def test_error_handling_1d():
    """
    Test that invalid inputs raise the expected exceptions in 1D.

    Checks two error paths:

    1. :meth:`Metric.riemannian_volume` must raise ``ValueError`` when called
       with an unsupported ``method`` string (e.g. ``'monte_carlo'``).
    2. :func:`geodesic_solver` must raise ``ValueError`` when called with an
       unrecognised ``method`` string (e.g. ``'magic'``).
    """
    x = symbols('x', real=True)
    m = Metric(1, (x,))
...
def test_euclidean_metric():
    """
    Test that the flat Euclidean metric g = I₂ produces straight geodesics.

    Verifies that the Gaussian curvature is zero and that the x-component of
    the geodesic trajectory (initial velocity (1, 1)) is approximately linear
    in t, consistent with the straight-line solution x(t) = t.
    """
    x, y = symbols('x y', real=True)
    m = Metric(Matrix([[1, 0], [0, 1]]), (x, y))
...
def test_polar_coordinates():
    """
    Test the polar coordinate metric g = diag(1, r²) in 2D.

    The polar metric is flat (it is merely the Euclidean metric expressed in
    polar coordinates), so the Gaussian curvature must be identically zero.
    Also verifies that the Laplace–Beltrami principal symbol equals
    ξᵣ² + ξ_θ²/r², which is the standard expression for the Laplacian in polar
    coordinates.
    """
    r, theta = symbols('r theta', real=True, positive=True)
    m = Metric(Matrix([[1, 0], [0, r**2]]), (r, theta))
...
def test_sphere_metric():
    """
    Test curvature computation for the unit sphere metric g = diag(1, sin²θ).

    Prints the Gaussian curvature K and the Ricci scalar R.  For the unit sphere,
    K = 1 everywhere and R = 2K = 2.  This test is primarily a smoke test to
    confirm that the curvature pipeline completes without error on a
    non-trivial metric.
    """
    theta, phi = symbols('theta phi', real=True)
    m = Metric(Matrix([[1, 0], [0, sin(theta)**2]]), (theta, phi))
...
def test_poincare_half_plane():
    """
    Test the Poincaré half-plane metric g = diag(1/y², 1/y²).

    Verifies that the Gaussian curvature is exactly −1 (constant negative
    curvature), and that a vertical geodesic starting at (0, 1) with initial
    velocity (0, 1) stays on the line x = 0 and moves upward (y(T) > 1), as
    expected for a vertical geodesic in the Poincaré upper half-plane model.
    """
    x, y = symbols('x y', real=True)
    m = Metric(Matrix([[1/y**2, 0], [0, 1/y**2]]), (x, y))
...
def test_hamiltonian_construction():
    """
    Test :meth:`Metric.from_hamiltonian` for the 2D polar kinetic energy.

    The Hamiltonian H = (p_r² + p_θ²/r²) / 2 corresponds to the polar
    coordinate metric g = diag(1, r²).  Verifies that the recovered metric
    matrix equals diag(1, r²) exactly (up to symbolic simplification).
    """
    r, theta = symbols('r theta', real=True, positive=True)
    pr, pt = symbols('p_r p_theta', real=True)
    H = (pr**2 + pt**2/r**2) / 2
...
def test_geodesic_integrators_2d():
    """
    Test symplectic energy conservation and arc-length reparametrisation in 2D.

    Uses the flat Euclidean metric g = I₂:

    1. **Symplectic integrator**: runs for 10 time units with 100 steps and
       checks that the standard deviation of the energy array is below 1e-3.
       Exact energy conservation is not expected (shadow Hamiltonian), but
       oscillations must remain bounded.
    2. **RK45 with reparametrisation**: verifies that the ``'arc_length'`` key
       is present in the output and that the accumulated arc length at t = 10
       approximates √2 · 10 ≈ 14.14 (geodesic of velocity (1, 1)) to 0.1%.
    """
    x, y = symbols('x y', real=True)
    m = Metric(Matrix([[1, 0], [0, 1]]), (x, y))
...
def test_distance_and_exp_map():
    """
    Test the exponential map and geodesic distance on the flat Euclidean metric.

    For g = I₂:

    * ``exponential_map(m, (0,0), (3,4), t=1)`` must return (3, 4) to within 1e-4.
    * ``distance(m, (0,0), (3,4), method='shooting')`` must return 5.0 to 0.1%.
    * ``distance(m, (0,0), (3,4), method='optimize')`` must return 5.0 to 5%.
      (The optimisation method is less precise but more robust for large separations.)
    """
    x, y = symbols('x y', real=True)
    m = Metric(Matrix([[1, 0], [0, 1]]), (x, y))
...
def test_hodge_star_and_volume():
    """
    Test the Hodge star operator and Riemannian volume for g = diag(4, 9).

    The metric g = diag(4, 9) has √|det g| = 6.  Verifies:

    * Symbolic and numerical volume of the unit square [0,1]² both equal 6.
    * The degree-2 Hodge star ⋆(12 dx∧dy) = 12 / √|det g| = 12/6 = 2.
    """
    x, y = symbols('x y', real=True)
    m = Metric(Matrix([[4, 0], [0, 9]]), (x, y))
...
def test_jacobi_field_stability():
    """
    Test Jacobi field stability on the unit sphere.

    Integrates the Jacobi equation along a geodesic on the unit sphere
    (g = diag(1, sin²θ)) starting from the equator (θ = π/2, φ = 0) with
    velocity (0, 1).  The initial Jacobi field is J₀ = (0, 0) with
    derivative DJ₀ = (0.1, 0).

    For a compact positively-curved space, Jacobi fields must remain bounded
    over finite time intervals.  The test asserts that max|J_x(t)| < 1.0
    over the interval [0, 2], consistent with bounded conjugate-point behaviour
    on the sphere.
    """
    theta, phi = symbols('theta phi', real=True)
    m = Metric(Matrix([[1, 0], [0, sin(theta)**2]]), (theta, phi))
