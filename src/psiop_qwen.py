...
#                return None
    ...
    def apply_peetre(
        ...
        The local polynomial part is now represented as pairs (a(x), q(xi))
        and applied using the same efficient strategy as separable terms:

            a(x) * q(D) u

        If a decomposition is provided, it is used directly and the
        decomposition-related parameters are ignored.
        """
        import numpy as np
        ...
        # Preferred form:
        #
        #     deco["local_terms"] = [(a_1, q_1), (a_2, q_2), ...]
        #
        # where each q_i is polynomial in the frequency variables.
        #
        # For backward compatibility, if local_terms is absent, rebuild it
        # from deco["local"] if possible.
        # --------------------------------------------------------------
        ...
        if local_terms is None or not local_terms:
            if deco.get("local", None):
                ...
                #
                # Very old/custom decomposition format:
                # only local_symbol is available. Fall back to direct
                # application of the full local symbol.
                #
        ...
        def _apply_separable_pair(a, q):
            ...
            This is exact for Kohn-Nirenberg quantization and is the
            efficient path because Op(q) has no spatial dependence.
            """
        ...
        #
        # It is applied exactly like separable terms:
        #
        #     a(x) q(D) u
        #
        # This avoids applying the whole variable-coefficient polynomial
        # symbol through the slower fully x-dependent KN path.
        # --------------------------------------------------------------
        for a, q in local_terms:
            result = result + _apply_separable_pair(a, q)

        # --------------------------------------------------------------
        # 2. Separable non-local terms: a(x) q(D) u
        # --------------------------------------------------------------
    ...
    def apply_peetre_old(
        ...
        Now supports Taylor refinement and asymptotic refinement.

        If decomposition is provided, it is used directly and the
        decomposition-related parameters are ignored.
        """
        import numpy as np
        from sympy import lambdify
        ...
        # 1. Local polynomial part
        # --------------------------------------------------------------
        if not self._peetre_is_zero(deco["local_symbol"]):
            op_local = PseudoDifferentialOperator(
                deco["local_symbol"],
                self.vars_x,
                mode="symbol",
        ...
        for a, q in deco["separable"]:
            ...
            try:
                ...
                result = result + np.asarray(a_vals) * v

            except Exception as exc:
                warnings.warn(
                    "Could not lambdify a separable spatial amplitude. "
                    f"Falling back to full separable symbol application: {exc}"
                )
        ...
        # 3. Joint residual
        # --------------------------------------------------------------
        if not self._peetre_is_zero(deco["joint_symbol"]):
            if apply_joint:
                op_joint = PseudoDifferentialOperator(
                    deco["joint_symbol"],
                    self.vars_x,
