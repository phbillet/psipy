...
class PseudoDifferentialOperator:
    ...
    def apply_subdomain(self, u, geometry, f, h=None,
                        x_grid=None, kx=None, **kwargs):
        """
        Convenience wrapper: build a SubdomainBoundaryPipeline and apply.

        Parameters
        ----------
        u : ndarray
        geometry : SubdomainGeometry
        f : Dirichlet data on C.
        h : Neumann data on C (optional).
        x_grid, kx : grids forwarded to pipeline.apply().

        Returns
        -------
        (v_Omega, diagnostics)
        """
        pipe = SubdomainBoundaryPipeline(self, geometry, f, h=h, **kwargs)
        return pipe.apply(u, x_grid, kx, **kwargs)
...
def _validate_pipeline_scope(symbol, freq_syms, tol=1e-8):
    """
    Reject genuinely nonlocal symbols before the pipeline is instantiated.

    A symbol is admissible if every term is polynomial in the frequency
    variables (local / differential) or a rational function with polynomial
    numerator and denominator (weakly nonlocal, rapidly decaying kernel).
    Pure fractional powers |ξ|^s with non-integer s are rejected.

    Parameters
    ----------
    symbol : sympy.Expr
        The operator symbol.
    freq_syms : tuple of sympy.Symbol
        Frequency variables, e.g. (xi,) or (xi, eta).
    tol : float
        Tolerance for the integer-exponent check.
    ...
    ------
    NotImplementedError
        If the symbol contains a genuinely nonlocal (fractional-power) term.
    """
    from sympy import Add, expand, Pow
    for term in Add.make_args(expand(symbol)):
        for sym, power in term.as_powers_dict().items():
            if sym in freq_syms:
                p_val = float(power)
                if abs(p_val - round(p_val)) > tol:
                    raise NotImplementedError(
                        f"Symbol contains fractional power {sym}**{power}. "
                        f"Genuinely nonlocal symbols (e.g. fractional "
                        f"Laplacian) are out of scope for this pipeline: "
                        f"the correct boundary-value problem requires "
                        f"prescribing data on the entire exterior of Ω, "
                        f"not just on C."
                    )
    return True


def _evaluate_on_grid(data, x_grid, y_grid=None, dim=1):
    """
    Evaluate boundary data (scalar, callable, or array) on the grid.

    Parameters
    ----------
    data : scalar, callable, or ndarray
        If scalar, broadcast. If callable, evaluate on the grid.
        If ndarray, use as-is.
    x_grid : ndarray
        Spatial grid in x.
    y_grid : ndarray, optional
    ...
    -------
    ndarray
        Data evaluated on the grid, dtype complex128.
    """
    import numpy as np
    if np.isscalar(data):
        if dim == 1:
            return np.full_like(x_grid, data, dtype=np.complex128)
        X, Y = np.meshgrid(x_grid, y_grid, indexing='ij')
        return np.full_like(X, data, dtype=np.complex128)
    if callable(data):
        if dim == 1:
            return np.asarray(data(x_grid), dtype=np.complex128)
        X, Y = np.meshgrid(x_grid, y_grid, indexing='ij')
        return np.asarray(data(X, Y), dtype=np.complex128)
    return np.asarray(data, dtype=np.complex128)


class SubdomainGeometry:
    """
    Diffuse-interface geometry for a subdomain Ω ⊂ 𝕋^d defined by g(x) ≤ 0.

    Pre-computes:
      χ_Ω   – smooth indicator mask (FFT Gaussian smoothing of 1_{g≤0})
      n_δ   – normal shell  ∇χ_Ω
      ρ_δ   – single-layer shell  |∇χ_Ω|   (Revision 2: derived from n_δ)
      n̂     – outward normal field  ∇g / |∇g|

    Parameters
    ----------
    x_grid : ndarray
        1-D spatial grid on [-L, L].
    g_func : callable
        Level-set function. g(x) ≤ 0 ⟺ x ∈ Ω.
        1-D signature: g_func(x) → array.
        2-D signature: g_func(X, Y) → array (meshgrid inputs).
    dim : int
        1 or 2.
    sigma : float or None
        Gaussian smoothing width. Auto-tuned as 2.5·Δx when None.
    y_grid : ndarray, optional
        1-D spatial grid in y (required for dim=2).
    """

    def __init__(self, x_grid, g_func, dim, sigma=None, y_grid=None):
        import numpy as np
        ...
        if dim == 2:
            if y_grid is None:
                raise ValueError("y_grid is required for dim=2.")
        ...
        self.sigma = 2.5 * self.dx if sigma is None else sigma
        self._precompute()

    # ------------------------------------------------------------------
    def _precompute(self):
        """Build χ_Ω, n_δ, ρ_δ, n̂ via FFT smoothing and spectral derivatives."""
        import numpy as np
        ...
        if self.dim == 1:
            N = len(self.x_grid)
            dx = self.dx
            g_vals = np.asarray(self.g_func(self.x_grid), dtype=float)
            indicator = (g_vals <= 0).astype(float)

            kx = 2.0 * np.pi * np.fft.fftfreq(N, d=dx)
            gauss = np.exp(-0.5 * self.sigma ** 2 * kx ** 2)
            self.chi = np.clip(np.real(ifft(fft(indicator) * gauss)), 0.0, 1.0)

            chi_hat = fft(self.chi)
            self.n_delta = np.real(ifft(1j * kx * chi_hat))
            self.rho_delta = np.abs(self.n_delta)

            dg = np.gradient(g_vals, dx)
            mag = np.abs(dg)
            self.n_hat = np.where(mag > 1e-14, dg / mag, 0.0)
            self.kx = kx
            ...
            g_vals = np.asarray(self.g_func(X, Y), dtype=float)
            indicator = (g_vals <= 0).astype(float)

            kx = 2.0 * np.pi * np.fft.fftfreq(Nx, d=dx)
            ky = 2.0 * np.pi * np.fft.fftfreq(Ny, d=dy)
            KX, KY = np.meshgrid(kx, ky, indexing='ij')
            gauss = np.exp(-0.5 * self.sigma ** 2 * (KX ** 2 + KY ** 2))
            self.chi = np.clip(np.real(ifft2(fft2(indicator) * gauss)), 0.0, 1.0)

            chi_hat = fft2(self.chi)
            ndx = np.real(ifft2(1j * KX * chi_hat))
            ndy = np.real(ifft2(1j * KY * chi_hat))
            self.n_delta = (ndx, ndy)
            self.rho_delta = np.sqrt(ndx ** 2 + ndy ** 2)

            dgx = np.gradient(g_vals, dx, axis=0)
            dgy = np.gradient(g_vals, dy, axis=1)
            mag = np.sqrt(dgx ** 2 + dgy ** 2) + 1e-14
            self.n_hat = (dgx / mag, dgy / mag)
            self.kx, self.ky = kx, ky
            self.KX, self.KY = KX, KY

    # ------------------------------------------------------------------
    def check_resolution(self, min_pts=5):
        """Warn if σ is resolved by fewer than *min_pts* grid points."""
        import warnings
        pts = self.sigma / self.dx
        if pts < min_pts:
            warnings.warn(
                f"σ={self.sigma:.4e} resolved by {pts:.1f} pts "
                f"(target ≥{min_pts}). Refine grid or increase σ.",
                stacklevel=2)
            return False
        return True

    # ------------------------------------------------------------------
    def trace_sample(self, field, weight=None):
        """
        ρ_δ-weighted average of *field* over the collar {ρ_δ > ε}.
        Returns a single complex scalar (the diffuse trace).
        """
        import numpy as np
        w = self.rho_delta if weight is None else weight
        eps = 1e-10 * np.max(w)
        mask = w > eps
        if not np.any(mask):
            return 0.0 + 0.0j
        return np.sum(field[mask] * w[mask]) / np.sum(w[mask])


class SubdomainBoundaryPipeline:
    """
    Kohn–Nirenberg Subdomain Boundary Integration Pipeline (Revision 2).

    Applies Op_KN(s) to u restricted to Ω with boundary data (f, h),
    entirely within a periodic Fourier framework.  Boundary conditions are
    injected via diffuse-interface shells and refined by a defect-correction
    loop (Step 7).

    Parameters
    ----------
    op : PseudoDifferentialOperator
        Operator to apply (KN or Weyl; Weyl is converted internally).
    geometry : SubdomainGeometry
        Pre-computed geometry (χ_Ω, ρ_δ, n_δ, n̂).
    f : scalar, callable, or ndarray
        Dirichlet data on C.
    h : scalar, callable, or ndarray, optional
        Neumann data on C.
    kappa_f, kappa_h : float
        Initial injection gains (refined by defect correction).
    max_defect_iters : int
        Iteration budget for the defect-correction loop.
    defect_tol : float
        Convergence tolerance on |residual_D|.
    defect_mode : {'gain', 'full'}
        'gain'  – secant update of the scalar gain κ_f.
        'full'  – pointwise update f_ext ← f_ext − residual_D.
    """

    def __init__(self, op, geometry, f, h=None,
                 kappa_f=1.0, kappa_h=1.0,
                 max_defect_iters=5, defect_tol=1e-6,
                 defect_mode='gain'):
        self.op = op
        self.geo = geometry
        self.f = f
        ...
        self.max_iters = max_defect_iters
        self.tol = defect_tol
        self.defect_mode = defect_mode

        # Scope gate
        _validate_pipeline_scope(op.symbol, op._peetre_frequency_symbols())

        # Pre-evaluate boundary data on the grid
        dim = geometry.dim
        self.f_ext = _evaluate_on_grid(f, geometry.x_grid, geometry.y_grid, dim)
        self.h_ext = (_evaluate_on_grid(h, geometry.x_grid, geometry.y_grid, dim)
                      if h is not None else None)

    # ------------------------------------------------------------------
    #  Core 7-step algorithm
    # ------------------------------------------------------------------
    def apply(self, u, x_grid, kx, defect_correction=True,
              y_grid=None, ky=None, **apply_kw):
        """
        Apply Op_KN(s) to u|_Ω with boundary data (f, h).

        Returns
        -------
        v_Omega : ndarray
        diagnostics : dict
            Keys: residual_D, residual_N, n_iters, converged,
                  kappa_f_history, kappa_h_history.
        """
        import numpy as np

        u_tot = self._build_input(u, self.kappa_f, self.kappa_h)
        v_gl = self.op.apply(u_tot, x_grid, kx,
                             boundary_condition='periodic',
                             y_grid=y_grid, ky=ky, **apply_kw)
        v_Omega = v_gl * self.geo.chi

        diag = dict(residual_D=None, residual_N=None,
                    n_iters=0, converged=True,
                    kappa_f_history=[self.kappa_f],
                    kappa_h_history=[self.kappa_h])

        if defect_correction:
            v_Omega, diag = self._defect_loop(
                u, x_grid, kx, y_grid=y_grid, ky=ky,
                v_Omega=v_Omega, **apply_kw)

        return v_Omega, diag

    # ------------------------------------------------------------------
    def _build_input(self, u, kf, kh):
        """Steps 1–4: assemble u_tot = u_Ω + F_shell + H_shell."""
        import numpy as np
        g = self.geo
        u_Omega = u * g.chi                          # Step 1
        F_shell = kf * self.f_ext * g.rho_delta      # Step 2

        if self.h_ext is not None:                   # Step 3
            if g.dim == 1:
                H_shell = kh * self.h_ext * (g.n_hat * g.n_delta)
            else:
                H_shell = kh * self.h_ext * (
                    g.n_hat[0] * g.n_delta[0] +
                    g.n_hat[1] * g.n_delta[1])
        else:
            H_shell = 0.0

        return u_Omega + F_shell + H_shell           # Step 4

    # ------------------------------------------------------------------
    #  Step 7 – defect correction
    # ------------------------------------------------------------------
    def _defect_loop(self, u, x_grid, kx, y_grid=None, ky=None,
                     v_Omega=None, **apply_kw):
        import numpy as np
        kf, kh = self.kappa_f, self.kappa_h
        kf_hist, kh_hist = [kf], [kh]
        ...
        for it in range(self.max_iters):
            ...
            if v_Omega is None:
                u_tot = self._build_input(u, kf, kh)
                v_gl = self.op.apply(u_tot, x_grid, kx,
                                     boundary_condition='periodic',
                                     y_grid=y_grid, ky=ky, **apply_kw)
                v_Omega = v_gl * self.geo.chi

            residual_D = self._trace_residual(v_Omega)
            if np.abs(residual_D) < self.tol:
                converged = True
                break

            if self.defect_mode == 'gain':
                delta = 0.1 * max(abs(kf), 1.0)
                u_p = self._build_input(u, kf + delta, kh)
                v_p = self.op.apply(u_p, x_grid, kx,
                                    boundary_condition='periodic',
                                    y_grid=y_grid, ky=ky, **apply_kw)
                res_p = self._trace_residual(v_p * self.geo.chi)
                if abs(res_p - residual_D) > 1e-14:
                    kf -= residual_D * delta / (res_p - residual_D)

            elif self.defect_mode == 'full':
                self.f_ext = self.f_ext - residual_D
        ...
        diag = dict(residual_D=residual_D, residual_N=None,
                    n_iters=n_iters, converged=converged,
                    kappa_f_history=kf_hist,
                    kappa_h_history=kh_hist)
        return v_Omega, diag

    # ------------------------------------------------------------------
    def _trace_residual(self, v_Omega):
        """residual_D = trace(v_Ω) − trace(f)."""
        return (self.geo.trace_sample(v_Omega)
                - self.geo.trace_sample(self.f_ext))

    # ------------------------------------------------------------------
    #  Validation helpers
    # ------------------------------------------------------------------
    def scope_check(self):
        """Print PASS / FAIL for the scope restriction."""
        try:
            _validate_pipeline_scope(self.op.symbol,
                                     self.op._peetre_frequency_symbols())
            print("✓ Scope check PASSED: symbol is local or weakly nonlocal.")
            return True
        except NotImplementedError as e:
            print(f"✗ Scope check FAILED: {e}")
            return False

    def resolution_sweep(self, N_values, u, f, h=None,
                         sigma_values=None, L=2.0, **apply_kw):
        """
        Refine N (and optionally σ) and report ‖residual_D‖.

        Parameters
        ----------
        N_values : list of int
        u : callable or ndarray
            Interior field (re-evaluated on each grid).
        f : scalar or callable
            Dirichlet data.
        h : scalar or callable, optional
        ...
        for N in N_values:
            ...
            for sig in sigmas:
                geo = SubdomainGeometry(x, self.geo.g_func, 1,
                                        sigma=sig)
                pipe = SubdomainBoundaryPipeline(
                    self.op, geo, f, h=h,
                    max_defect_iters=self.max_iters,
                    defect_tol=self.tol,
                    defect_mode=self.defect_mode)

                u_vals = (_evaluate_on_grid(u, x, dim=1)
                          if callable(u) else np.asarray(u, dtype=complex))
                try:
                    _, diag = pipe.apply(u_vals, x, k, **apply_kw)
                    results.append(dict(N=N, sigma=geo.sigma,
                                        residual_D=diag['residual_D'],
                                        n_iters=diag['n_iters'],
                                        converged=diag['converged']))
                except Exception as exc:
                    results.append(dict(N=N, sigma=geo.sigma,
                                        residual_D=None, error=str(exc)))

        hdr = f"{'N':>6} {'σ':>12} {'|res_D|':>14} {'iters':>6} {'conv':>6}"
        print(f"\n{hdr}\n{'-'*len(hdr)}")
        for r in results:
            rd = r.get('residual_D')
            rd_s = f"{abs(rd):.6e}" if rd is not None else "N/A"
            print(f"{r['N']:>6} {r['sigma']:>12.4e} {rd_s:>14} "
                  f"{r.get('n_iters',0):>6} {str(r.get('converged','')):>6}")
        return results

    def validate_laplacian_1d(self, N=256, L=2.0, a=-1.0, b=1.0, f_val=1.0):
        """
        Known-solution regression test: 1-D Laplacian on [a, b].

        Operator  : ξ²  (i.e. −d²/dx²)
        Exact     : u(x) = f_val  (constant) ⇒ −u″ = 0
        Check     : residual_D → 0 under refinement.

        Returns
        -------
        dict  with residual_D, n_iters, converged, L2_error.
        """
        import numpy as np
        from sympy import symbols
        ...
        g_func = lambda xv: (xv - a) * (xv - b)     # ≤ 0 on [a,b]
        geo = SubdomainGeometry(x, g_func, dim=1)

        xi = symbols('xi', real=True)
        lap = PseudoDifferentialOperator(xi ** 2, [symbols('x', real=True)],
                                         mode='symbol')

        pipe = SubdomainBoundaryPipeline(lap, geo, f=f_val,
                                         max_defect_iters=self.max_iters,
                                         defect_tol=self.tol,
                                         defect_mode=self.defect_mode)

        u_vals = np.full(N, f_val, dtype=complex)
        v_Omega, diag = pipe.apply(u_vals, x, k)

        exact = np.full(N, 0.0, dtype=complex)       # −u″ = 0
        L2 = np.sqrt(np.mean(np.abs(v_Omega - exact) ** 2))

        print(f"validate_laplacian_1d:  |res_D|={abs(diag['residual_D']):.4e}  "
              f"L2={L2:.4e}  iters={diag['n_iters']}  "
              f"converged={diag['converged']}")
        return dict(residual_D=diag['residual_D'], L2_error=L2,
                    n_iters=diag['n_iters'], converged=diag['converged'])
