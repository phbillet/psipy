...
class PseudoDifferentialOperator:
    ...
    def __init__(self, expr, vars_x, var_u=None, mode='symbol', quantization='kohn-nirenberg',
        ...
        self.decompose_order = decompose_order
        self._decomposition_cache = {}
        self._effective_symbol_func_cache = {}
...
#            if not isinstance(expr, (int, float, complex)):
            ...
            self.xi, self.eta = xi_internal, eta_internal
#            if not isinstance(expr, (int, float, complex)):
            expr = sympify(expr)
            expr = expr.subs(symbols('xi', real=True), xi_internal)
        ...
        # --- Symbol decomposition (local / separable / joint) ---
        # Computed eagerly here for 'kohn-nirenberg' (the effective symbol is
        # known now: self.symbol). For 'weyl', the effective symbol depends on
        # weyl_order (a per-call parameter of apply()), so we prime the cache
        # with the same default (4) used by apply(); other weyl_order values
        # are decomposed lazily on first use and cached the same way — same
        # treatment, same code path, for both quantizations.
        self._get_decomposition(
            order=self.decompose_order,
            weyl_order=4 if self.quantization == 'weyl' else None,
        )
    ...
    def apply(
        ...
        space_window=False,
        weyl_order=4,
        nonperiodic_decomposition=False,
    ):
        """
        Apply the pseudo-differential operator to u.
    
        Periodic branch
        ---------------
        Uses the local/separable/joint decomposition of the effective symbol.
    
        If the joint residual is empty, the local and separable parts are applied
        by exact FFT-based spectral shortcuts.
    
        If a joint residual exists, the full effective symbol is passed to the
        periodic Kohn-Nirenberg backend. This is intentional: the slow periodic
        backend cost is dominated by the grid size, not by symbol complexity.
    
        Non-periodic branch
        -------------------
        By default, this method preserves the previous behavior: the full effective
        symbol is passed to kohn_nirenberg_nonperiodic.
    
        If nonperiodic_decomposition=True, the effective symbol decomposition is
        used:
    
            p_eff = p_local + sum_i a_i(x) q_i(xi) + p_joint
    
        where p_eff is the effective Kohn-Nirenberg symbol. For Weyl quantization,
        p_eff is obtained from the Weyl symbol via weyl_to_kn_symbol().
    
        This avoids the main theoretical problem of applying raw Weyl separable
        pieces as a(x) Op^Weyl(q), which is not the Weyl quantization of a(x)q(xi).
    
        Notes
        -----
        The non-periodic decomposed path is not necessarily faster. It can even be
        slower because several non-periodic backend calls may be made.
    
        Also, because the numerical backend contains nonlinear magnitude clipping,
        piecewise application with clamp active is not exactly equivalent to
        applying the full symbol at once. If exact equivalence with the full-symbol
        backend is required, keep nonperiodic_decomposition=False or set clamp=np.inf.
        """
        ...
        # ------------------------------------------------------------------
        # Small helper for the non-periodic backend
        # ------------------------------------------------------------------
        def _call_nonperiodic_backend(symbol_func, is_spatial, use_space_window):
            if self.dim == 1:
        ...
        if boundary_condition == 'periodic':
            decomp = self._get_decomposition(
                order=self.decompose_order,
                weyl_order=weyl_order if self.quantization == 'weyl' else None,
            )
    
            # If the joint residual is exactly zero, the operator is fully
            # local + separable on the effective symbol. Use exact FFT shortcuts.
            if decomp['joint_expr'] == 0:
                ...
                result = result + self._apply_separable(
                    u,
                    x_grid,
                    kx,
                    y_grid,
                    ky,
                    dealiasing_mask,
                    decomp['separable_funcs'],
                )
    
                return result
    
            # Otherwise, fall back to the full effective symbol.
            #
            # This is also the safe fallback if a joint residual exists but could
            # not be lambdified: decomp['joint_symbol_func'] may be None while
            # decomp['joint_expr'] != 0.
        ...
        # ==================================================================
        # 2. Non-periodic boundary conditions
        # ==================================================================
        if boundary_condition in {'dirichlet', 'neumann'}:
            ...
            # --------------------------------------------------------------
            # Full-symbol non-periodic fallback
            # --------------------------------------------------------------
            def _full_nonperiodic():
                symbol_func = self._get_effective_symbol_func(weyl_order=weyl_order)
                is_spatial = self._is_spatial_dependent()
    
                return _call_nonperiodic_backend(
                    symbol_func=symbol_func,
                    is_spatial=is_spatial,
                    use_space_window=space_window,
                )
    
            # Default behavior: preserve the old full-symbol backend.
            if not nonperiodic_decomposition:
                return _full_nonperiodic()
    
            # If a joint residual exists but could not be lambdified, do not
            # attempt a piecewise application. Fall back to the full symbol.
            if decomp['joint_expr'] != 0 and decomp['joint_symbol_func'] is None:
                warnings.warn(
                    "Non-periodic decomposed application requested, but the joint "
                    "residual could not be lambdified. Falling back to the full "
                    "effective symbol.",
                    stacklevel=2,
                )
                return _full_nonperiodic()
            ...
            # --------------------------------------------------------------
            # Decomposed non-periodic application
            #
            # Important:
            # The decomposition returned by _get_decomposition() is a
            # decomposition of the effective Kohn-Nirenberg symbol.
            #
            # For Weyl quantization, this means the Weyl -> KN correction has
            # already been applied. Therefore we do NOT create new
            # PseudoDifferentialOperator objects with quantization=self.quantization.
            # That could double-apply the Weyl correction or use the wrong
            # quantization convention for separable pieces.
            # --------------------------------------------------------------
            result = np.zeros_like(u, dtype=np.complex128)
    
            # --------------------------------------------------------------
            # 2.1 Local polynomial part
            # --------------------------------------------------------------
            if decomp['local_funcs']:
    
                if self.dim == 1:
                    def local_symbol_func(x, xi):
                        out = 0.0
                        for (k,), coeff_f in decomp['local_funcs']:
                            out = out + coeff_f(x) * (xi ** k)
                        return out
    
                else:
                    def local_symbol_func(x, y, xi, eta):
                        out = 0.0
                        for (k1, k2), coeff_f in decomp['local_funcs']:
                            out = out + coeff_f(x, y) * (xi ** k1) * (eta ** k2)
                        return out
    
                # Determine whether the local part should be treated as
                # x-dependent. If space_window is True, we force the slow path
                # so that the spatial taper is applied consistently.
                local_is_spatial = space_window
    
                if not local_is_spatial:
                    for coeff_expr in decomp['local_coeffs_sym'].values():
                        if coeff_expr.has(*self.vars_x):
                            local_is_spatial = True
                            break
    
                result = result + _call_nonperiodic_backend(
                    symbol_func=local_symbol_func,
                    is_spatial=local_is_spatial,
                    use_space_window=space_window,
                )
    
            # --------------------------------------------------------------
            # 2.2 Separable part: sum_i a_i(x) q_i(D) u
            #
            # Since the decomposition is performed on the effective KN symbol,
            # applying q_i(D) and then multiplying by a_i(x) is valid at the
            # KN level.
            #
            # For space_window=True, we apply the spatial taper manually after
            # q_i(D), because the x-independent fast path of the 2D non-periodic
            # backend does not apply space_window.
            # --------------------------------------------------------------
            if decomp['separable_funcs']:
    
                if self.dim == 1:
    
                    if space_window:
                        x_center = (x_grid[0] + x_grid[-1]) / 2.0
                        L_half = (x_grid[-1] - x_grid[0]) / 2.0
                        sw = np.exp(-((x_grid - x_center) / L_half) ** 2)
                    else:
                        sw = 1.0
    
                    for a_f, q_f in decomp['separable_funcs']:
    
                        def q_symbol_func(x, xi, q_f=q_f):
                            return q_f(xi)
    
                        q_u = _call_nonperiodic_backend(
                            symbol_func=q_symbol_func,
                            is_spatial=False,
                            use_space_window=False,
                        )
    
                        a_vals = np.asarray(a_f(x_grid))
    
                        result = result + sw * a_vals * q_u
                    ...
                    if space_window:
                        ...
                        sw_x = np.exp(-((x_grid - x_center) / Lx) ** 2)
    
                        y_center = (y_grid[0] + y_grid[-1]) / 2.0
                        Ly = (y_grid[-1] - y_grid[0]) / 2.0
                        sw_y = np.exp(-((y_grid - y_center) / Ly) ** 2)
    
                        sw = sw_x[:, None] * sw_y[None, :]
                    else:
                        sw = 1.0
    
                    for a_f, q_f in decomp['separable_funcs']:
    
                        def q_symbol_func(x, y, xi, eta, q_f=q_f):
                            return q_f(xi, eta)
            ...
            if decomp['joint_expr'] != 0:
                result = result + _call_nonperiodic_backend(
                    symbol_func=decomp['joint_symbol_func'],
                    is_spatial=True,
                    use_space_window=space_window,
                )
    ...
    def apply_old(self, u, x_grid, kx, boundary_condition='periodic',
        ...
        """
        # Case 1 (periodic BC): dispatch entirely on the decomposition.
        #
        # A symbol with no x-dependence at all is mathematically a STRICT
        # special case of "joint residual empty" — if x never appears in
        # the expression, no term can ever satisfy `q.has(x)` in
        # `_classify_terms`, so it can never be routed to the joint
        # bucket. There is therefore no need to keep a separate
        # `not is_spatial` fast path (the old `_apply_constant_fft`
        # special case): the decomposition already generalizes it to any
        # symbol that is local + separable, x-dependent or not, with only
        # a negligible constant-factor cost (a few extra small FFTs when
        # several terms are involved — sub-millisecond in practice, see
        # accompanying benchmark).
        #
        # Use the local / separable / joint decomposition instead of calling
        # the full KN backend on the whole symbol: the local part (polynomial
        # in xi[,eta]) and the separable part (a(x)*q(xi) with q non-
        # polynomial) are both applied as EXACT spectral shortcuts; only a
        # genuinely entangled ("joint") residual — if any — goes through the
        # full O(N^2)/O(N^4) KN backend, UNCHANGED from the original
        # implementation (see note below on why partial mixing would not
        # help). Same decomposition machinery for 'kohn-nirenberg' and
        # 'weyl' (see _get_decomposition).
        #
        # freq_window / clamp / space_window are NOT applied to the local /
        # separable parts: those are exact (no numerical truncation), so
        # there is nothing to stabilize there. They are only passed to the
        # KN backend call below, exactly as before, for the joint residual.
        #
        # dealiasing_mask IS applied consistently everywhere a Fourier
        # multiplication occurs: in _apply_local, in _apply_separable, and
        # (via the dealiasing_mask parameter added to kohn_nirenberg_fft)
        # in the joint-residual backend call.
        #
        # This decomposition-based fast path assumes periodicity (spectral
        # differentiation via FFT bakes it in), so it is only valid here —
        # Case 2 (Dirichlet/Neumann) below deliberately keeps calling the
        # full-symbol non-periodic backend for ALL pieces, local included.
        if boundary_condition == 'periodic':
            decomp = self._get_decomposition(
                order=self.decompose_order,
                weyl_order=weyl_order if self.quantization == 'weyl' else None,
            ...
            # IMPORTANT: kohn_nirenberg_fft's slow path is a direct grid
            # quadrature whose cost depends only on grid size (O(N^2) in 1D,
            # O(N^4) in 2D) — NOT on how simple or complex symbol_func is.
            # So calling it on the joint residual costs exactly as much as
            # calling it on the full effective symbol. Adding the local /
            # separable contributions on top of that unavoidable call would
            # only ADD overhead, never remove any — a pure regression, not
            # a speed-up. The decomposition therefore only pays off in the
            # all-or-nothing case where the joint residual is empty (no call
            # to the expensive backend at all). Otherwise, fall back to the
            # original single-call behaviour, unchanged — which, with the
            # decomposition now catching every local/separable symbol
            # (including the old `not is_spatial` case), is reached far
            # less often than before: only for genuinely entangled symbols.
            if decomp['joint_symbol_func'] is None:
                result = self._apply_local(
                    u, x_grid, kx, y_grid, ky, dealiasing_mask, decomp['local_funcs']
                )
                result = result + self._apply_separable(
                    u, x_grid, kx, y_grid, ky, dealiasing_mask, decomp['separable_funcs']
                )
                return result
            ...
            return kohn_nirenberg_fft(
                ...
                clamp=clamp,
                space_window=space_window,
                is_spatial=True,  # genuinely x-dependent, by construction of the joint residual
                dealiasing_mask=dealiasing_mask,
            )

        # Case 2: Dirichlet/Neumann BC (non-periodic).
        #
        # The decomposition's local/separable fast paths assume periodic
        # spectral differentiation (FFT-based), which is NOT valid for a
        # non-periodic quadrature — so ALL pieces (local, separable, joint)
        # are kept together and passed as the single effective symbol to
        # the existing non-periodic backend, exactly as before. Only change:
        # dealiasing_mask is now threaded through here too.
    ...
    def _get_effective_symbol_func(self, weyl_order=4):
        ...
        Notes
        -----
        The corrected symbol (and its lambdified form) is cached per
        ``weyl_order`` in ``self._effective_symbol_func_cache``, so repeated
        ``apply()`` calls never pay the SymPy lambdify cost twice for the
        same effective symbol.
        """
        key = weyl_order if self.quantization == 'weyl' else None
        if key in self._effective_symbol_func_cache:
            return self._effective_symbol_func_cache[key]
        ...
        if self.dim == 1:
            x  = self.vars_x[0]
            xi = symbols('xi', real=True)
            func = lambdify((x, xi), effective_symbol, 'numpy')
     
        elif self.dim == 2:
            x, y    = self.vars_x
            xi, eta = symbols('xi eta', real=True)
            func = lambdify((x, y, xi, eta), effective_symbol, 'numpy')
        ...
        self._effective_symbol_func_cache[key] = func
        return func
...
#                return None
    ...
    def asymptotic_expansion(self, order=3, expr=None):
        """
        ...
        expr : sympy expression, optional
            If given, expand this expression instead of ``self.symbol`` (e.g.
            a single entangled term isolated by ``_classify_terms``). Useful
            to expand term-by-term so that one non-expandable term (e.g. a
            Gaussian, which decays faster than any power) does not block the
            expansion of the other terms.
        ...
        """
        p = expr if expr is not None else self.symbol
    ...
    # ------------------------------------------------------------------
    # Symbol decomposition: local (polynomial in xi[,eta]) / separable
    # (a(x[,y]) * q(xi[,eta]) with q non-polynomial) / joint (genuinely
    # entangled — no exact closed form in general). See Peetre's theorem
    # for the local/non-local boundary. Purely symbolic (SymPy), no SVD.
    # ------------------------------------------------------------------
    def _classify_terms(self, expr):
        """
        Split `expr` into:
          local_coeffs : dict {multi-index tuple: coeff expr in vars_x}
                         representing sum_alpha coeff(x[,y]) * xi**a1 [* eta**a2]
          separable    : list of (a_expr, q_expr), q depending only on the
                         frequency variable(s), not polynomial in them
          joint        : list of raw terms entangling space and frequency
        Exact, term-by-term structural classification (no approximation).
        """
        xi_vars = [self.xi] if self.dim == 1 else [self.xi, self.eta]
        x_vars = list(self.vars_x)
        ...
        if local_terms:
            ...
            for monom, c in poly.terms():
                local_coeffs[monom] = simplify(local_coeffs.get(monom, 0) + c)

        return local_coeffs, separable, joint

    def _decompose_expr(self, expr, order):
        """
        Full symbolic decomposition of `expr`:
          1. structural classification (`_classify_terms`);
          2. for each entangled ("joint") term, peel off further local /
             separable pieces via `self.asymptotic_expansion(expr=term,
             order=order)` (high-frequency classical symbol expansion,
             reusing the project's own asymptotic-expansion method — this
             replaces the ad hoc Taylor/asymptotic refinements used during
             the exploratory phase).
        The truncation error is kept as an EXACT symbolic remainder
        (term - expansion), never approximated numerically. Robust: if the
        expansion fails for a given term (e.g. a Gaussian, which decays
        faster than any power, or any other SymPy limitation), that term
        is simply kept unchanged in the joint residual — no crash, no
        silent loss of terms.
        """
        local_coeffs, separable, joint = self._classify_terms(expr)

        refined_joint = []
        for term in joint:
            try:
                expansion = self.asymptotic_expansion(order=order, expr=term)
            except Exception:
                refined_joint.append(term)
                continue

            remainder = expand(term - expansion)
            lc, sep, jt = self._classify_terms(expansion)

            for k, v in lc.items():
                local_coeffs[k] = simplify(local_coeffs.get(k, 0) + v)
            separable.extend(sep)
            # `jt` is normally empty (expansion coefficients depend on the
            # frequency variable(s) alone); if not — e.g. asymptotic_expansion
            # silently returned its input unchanged after an internal
            # failure — fold it back into the joint residual instead of
            # losing it.
            refined_joint.extend(jt)

            if remainder != 0:
                refined_joint.append(remainder)

        local_coeffs = {k: v for k, v in local_coeffs.items() if simplify(v) != 0}
        separable = [(a, q) for a, q in separable if simplify(a) != 0 and simplify(q) != 0]
        refined_joint = [t for t in refined_joint if simplify(t) != 0]

        return local_coeffs, separable, refined_joint

    def _lambdify_decomposition(self, local_coeffs, separable, joint_terms):
        """
        Turn the symbolic decomposition into ready-to-use NumPy callables,
        computed once (mirrors the existing `self.p_func` lambdify step).
        Falls back gracefully — the offending piece is folded back into
        the joint expression — if it cannot be lambdified, exactly like
        the existing `self.p_func = None` fallback for undefined-function
        symbols.
        """
        x_vars = tuple(self.vars_x)
        xi_vars = (self.xi,) if self.dim == 1 else (self.xi, self.eta)
        joint_terms = list(joint_terms)

        local_funcs = []
        for monom, coeff in local_coeffs.items():
            try:
                f = lambdify(x_vars, coeff, 'numpy')
                local_funcs.append((monom, f))
            except Exception:
                joint_terms.append(coeff * Mul(*[xv**k for xv, k in zip(xi_vars, monom)]))

        separable_funcs = []
        for a_expr, q_expr in separable:
            try:
                a_f = lambdify(x_vars, a_expr, 'numpy')
                q_f = lambdify(xi_vars, q_expr, 'numpy')
                separable_funcs.append((a_f, q_f))
            except Exception:
                joint_terms.append(a_expr * q_expr)

        joint_expr = Add(*joint_terms) if joint_terms else sympify(0)
        joint_symbol_func = None
        if joint_terms:
            try:
                joint_symbol_func = lambdify(x_vars + xi_vars, joint_expr, 'numpy')
            except Exception:
                joint_symbol_func = None  # ultimate fallback: caller must handle None

        return {
            'local_funcs': local_funcs,
            'separable_funcs': separable_funcs,
            'joint_symbol_func': joint_symbol_func,
            'joint_expr': joint_expr,
            'local_coeffs_sym': local_coeffs,
            'separable_sym': separable,
        }

    def _get_decomposition(self, order=None, weyl_order=None):
        """
        Return the (cached) symbolic decomposition of the *effective*
        symbol: the raw symbol for ``quantization='kohn-nirenberg'``, or
        its KN-equivalent (via ``weyl_to_kn_symbol``) for
        ``quantization='weyl'`` — same decomposition machinery for both,
        just applied to a different starting expression.

        Cached per ``(quantization, weyl_order, order)`` so repeated
        ``apply()`` calls — even with varying ``weyl_order`` — never
        recompute the same decomposition twice.
        """
        order = order if order is not None else self.decompose_order
        key = ('weyl', weyl_order, order) if self.quantization == 'weyl' else ('kn', order)
        ...
        try:
            if self.quantization == 'weyl':
                ...
                effective_symbol = self.symbol
            local_coeffs, separable, joint = self._decompose_expr(effective_symbol, order)
        except Exception as e:
            warnings.warn(
                f"Symbol decomposition failed ({e!r}); falling back to the "
                f"full symbol via the existing KN backend (no speed-up).",
                stacklevel=2,
            )
            local_coeffs, separable, joint = {}, [], [self.symbol]

        result = self._lambdify_decomposition(local_coeffs, separable, joint)
        self._decomposition_cache[key] = result
        return result

    def describe_decomposition(self, order=None, weyl_order=None, verbose=True):
        """
        Pretty-print the local / separable / joint decomposition of the
        (effective) symbol, and return it as a dict.
        """
        order = order if order is not None else self.decompose_order
        if weyl_order is None and self.quantization == 'weyl':
            weyl_order = 4
        decomp = self._get_decomposition(order=order, weyl_order=weyl_order)

        xi_vars = (self.xi,) if self.dim == 1 else (self.xi, self.eta)
        local_coeffs = decomp['local_coeffs_sym']
        separable = decomp['separable_sym']
        joint_expr = decomp['joint_expr']
        joint_terms = Add.make_args(joint_expr) if joint_expr != 0 else []

        n_local, n_sep, n_joint = len(local_coeffs), len(separable), len(joint_terms)

        if verbose:
            quant_label = 'Weyl' if self.quantization == 'weyl' else 'Kohn-Nirenberg'
            extra = f", weyl_order={weyl_order}" if self.quantization == 'weyl' else ""
            print("=" * 70)
            print(f"Decomposition of the {quant_label} symbol (order={order}{extra})")
            print("=" * 70)

            print(f"\n-- LOCAL part ({n_local} term(s), polynomial in {xi_vars}) --")
            if n_local == 0:
                print("  (none)")
            for monom, coeff in local_coeffs.items():
                mono_str = "*".join(f"{v}**{k}" for v, k in zip(xi_vars, monom) if k) or "1"
                print(f"  ({coeff}) * {mono_str}")

            print(f"\n-- SEPARABLE non-local part ({n_sep} term(s)) --")
            if n_sep == 0:
                print("  (none)")
            for a, q in separable:
                print(f"  ({a}) * ({q})")

            print(f"\n-- JOINT residual ({n_joint} term(s)) --")
            if n_joint == 0:
                print("  (none) -> fully exact fast path, no call to the KN backend")
            else:
                for t in joint_terms:
                    print(f"  {t}")
                print("  -> falls back to the full KN backend for this part (see apply())")
            print()

        return {
            'local_coeffs': local_coeffs,
            'separable': separable,
            'joint_residual': joint_terms,
            'n_local': n_local,
            'n_separable': n_sep,
            'n_joint': n_joint,
        }

    def _apply_local(self, u, x_grid, kx, y_grid, ky, dealiasing_mask, local_funcs):
        """
        Apply sum_alpha coeff_alpha(x[,y]) * D^alpha u spectrally.
        EXACT (no truncation), but assumes a periodic grid: spectral
        differentiation via FFT inherently bakes in periodicity, so this
        is only valid for boundary_condition='periodic'.
        """
        if not local_funcs:
        ...
        u_hat = self.fft(u)
        if dealiasing_mask is not None:
            u_hat = u_hat * dealiasing_mask

        out = np.zeros_like(u, dtype=complex)
        if self.dim == 1:
            # Op(xi**k) u = IFFT(xi**k * FFT(u))  -- NO extra (1j)**k factor:
            # with D = -i d/dx (the standard PsiDO convention), one has
            # xi**k * FFT(u) = FFT(D**k u) directly (verified numerically
            # against an exact derivative; see accompanying notes).
            for (k,), coeff_f in local_funcs:
                Dk_u = self.ifft((kx ** k) * u_hat)
                out = out + coeff_f(x_grid) * Dk_u
        else:
            KX, KY = np.meshgrid(kx, ky, indexing='ij')
            X, Y = np.meshgrid(x_grid, y_grid, indexing='ij')
            for (k1, k2), coeff_f in local_funcs:
                Dk_u = self.ifft((KX ** k1) * (KY ** k2) * u_hat)
                out = out + coeff_f(X, Y) * Dk_u
        return out

    def _apply_separable(self, u, x_grid, kx, y_grid, ky, dealiasing_mask, separable_funcs):
        """
        Apply sum_i a_i(x[,y]) * q_i(D) u as exact Fourier multipliers.
        EXACT (no truncation, no SVD), same periodicity assumption as
        `_apply_local` (valid only for boundary_condition='periodic').
        """
        if not separable_funcs:
            return np.zeros_like(u, dtype=complex)
        ...
        out = np.zeros_like(u, dtype=complex)
        if self.dim == 1:
            for a_f, q_f in separable_funcs:
                q_vals = np.broadcast_to(np.asarray(q_f(kx), dtype=complex), kx.shape)
                term = self.ifft(q_vals * u_hat)
                out = out + a_f(x_grid) * term
        else:
            KX, KY = np.meshgrid(kx, ky, indexing='ij')
            X, Y = np.meshgrid(x_grid, y_grid, indexing='ij')
            for a_f, q_f in separable_funcs:
                q_vals = np.broadcast_to(np.asarray(q_f(KX, KY), dtype=complex), KX.shape)
                term = self.ifft(q_vals * u_hat)
                out = out + a_f(X, Y) * term
        return out
    ...
    def fractional_power(self, alpha, order=1, method='symbolic', x_grid=None, L=None, N=None):
        ...
        """
        import sympy as sp
        from sympy import Rational, simplify, symbols, powdenest, nsimplify
        import numpy as np
        if method == 'numerical':
...
def kohn_nirenberg_fft(
    ...
    **Memory-Bounded Slow Path (Spatial Dependence)**
    ...
    if dim == 1:
        ...
        if dealiasing_mask is not None:
            f_hat = f_hat * np.fft.fftshift(dealiasing_mask)
