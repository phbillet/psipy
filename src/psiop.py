    ...
    def apply_subdomain(
        ...
        **apply_kwargs,
        ...
        correction : {'gain', 'full', None}, default 'gain'
            ...
            points are available. It can only rescale the *magnitude* of
            the injected data, though, not its shape, so it has an
            accuracy floor for a mismatched `f`. `'full'` does a
            `rho_delta`-preconditioned, relaxed fixed-point update of
            `f`/`h` directly (Picard iteration for the boundary integral
            equation this pipeline implicitly approximates). The
            preconditioning corrects for `rho_delta`'s `O(1/sigma)`
            peak amplitude, which otherwise makes a plain pointwise update
            get stiffer -- and can diverge -- as the grid is refined; it
            is still a heuristic Picard iteration with no guaranteed
            contraction, so treat convergence as problem-dependent and
            check `residual_D`/`residual_N` and `converged`, not just
            `n_iter == max_iter`. `None` performs a single forward pass,
            no correction.
        relax : float, default 0.5
            Damping factor for the `'full'` correction's preconditioned
            fixed-point update. Lower it (e.g. 0.1-0.3) if the residual
            grows across iterations. Ignored for `'gain'` and `None`.
            Regardless of `relax`, the iteration is monitored: if the
            residual grows past 10x its best value so far, it stops and
            the best iterate found is returned rather than a diverged one.
        ...
        Notes
        -----
        `f` is the boundary data **of `u`** (`u|_C = f`), not of the
        output -- Step 7 checks whether the operator's action on the
        reconstructed field reproduces that same trace, which is only a
        meaningful check for operators for which `Op_KN(s)(u)|_C ~ u|_C`
        is expected to roughly hold (elliptic resolvents, smoothing
        operators, near-identity multipliers) -- not, e.g., a bare
        differential operator like `xi**2`, whose boundary trace has no
        reason to resemble `u`'s. Do not expect `residual_D` to reach
        machine precision: it reflects (a) the diffuse-interface
        approximation itself, whose consistency error is `O(sigma)`, not
        spectral, and (b) for `'gain'`, the fact that a single scalar
        cannot fully correct a spatially-varying mismatch. What should
        improve is (i) `residual_D` versus the *uncorrected* baseline
        (`correction=None`) at fixed resolution, and (ii) both getting
        smaller under the resolution sweep in the Validation Protocol
        (refine `N`, then shrink `sigma` at fixed `N`) -- that sweep, not
        a single run, is the real trust check.
        """
        if not assume_local:
            warnings.warn(
        ...
        for it in range(1, max_iter + 1):
            ...
            if correction == 'gain':
                ...
                eps = 1e-2 * rho_delta.max() if rho_delta.max() > 0 else 1e-12
                precond = 1.0 / (rho_delta + eps)
                f_ext = f_ext - relax * residual_field * precond
                if h_ext is not None:
                    h_ext = h_ext - relax * residual_field_N * precond
