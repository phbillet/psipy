...
def test_stationary_equation_1d():
...
def test_stationary_equation_with_x_and_xi_1d():
    # Symbols
    x = symbols('x')
    xi = symbols('xi', real=True)
    u = Function('u')(x)
...
def test_diffusion_equation_1d():
    ...
    for i in range(n_test + 1):
        err = solver.test(u_exact=u_exact, t_eval=i * Lt / n_test, threshold=50, component='real')
        assert err is not None and err < 50
...
def test_wave_equation_1d():
    ...
    n_test = 10
    for i in range(n_test + 1):
        err = solver.test(u_exact=u_exact, t_eval=i * Lt / n_test, threshold=7e-1, component='real')
        assert err is not None and err < 7e-1
...
def test_equation_psiOp_depending_on_x_1d():
    # Symbols
    t, x, xi = symbols('t x xi', real=True)
    u = Function('u')
...
def test_hermite_equation_1d():
    ...
    for i in range(n_test + 1):
        t_eval = i * Lt / n_test
        err = solver.test(u_exact=u_exact, t_eval=t_eval, threshold=50, component='real')
        assert err is not None and err < 50

def test_airy_equation_1d():
    # Symbols
    t, x, xi = symbols('t x xi', real=True)
    u = Function('u')
